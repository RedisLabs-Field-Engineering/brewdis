...
import static com.redis.brewdis.BrewdisField.DELTA;
import static com.redis.brewdis.BrewdisField.EPOCH;
import static com.redis.brewdis.BrewdisField.LEVEL;
...
import static com.redis.brewdis.BrewdisField.RESERVED;
import static com.redis.brewdis.BrewdisField.STORE_ID;
import static com.redis.brewdis.BrewdisField.TIME;
import static com.redis.brewdis.BrewdisField.VIRTUAL_HOLD;
...
public class InventoryManager
	...
	private final Logger log = LoggerFactory.getLogger(InventoryManager.class);
	...
	public void afterPropertiesSet() throws Exception {
		...
		commands.ftCreate(index,
				CreateOptions.<String, String>builder()
						.prefix(config.getInventory().getKeyspace() + config.getKeySeparator()).build(),
				Field.tag(STORE_ID).sortable().build(), Field.tag(PRODUCT_ID).sortable().build(),
				Field.geo(LOCATION).build(), Field.numeric(AVAILABLE_TO_PROMISE).sortable().build(),
				Field.numeric(ON_HAND).sortable().build(), Field.numeric(ALLOCATED).sortable().build(),
				Field.numeric(RESERVED).sortable().build(), Field.numeric(VIRTUAL_HOLD).sortable().build(),
	...
	public void cleanup() {
		...
		SearchResults<String, String> results = commands.ftSearch(index, query,
				SearchOptions.<String, String>builder().noContent(true)
						.limit(Limit.offset(0).num(config.getInventory().getCleanup().getSearchLimit())).build());
		if (!results.isEmpty()) {
